                ...
                <tbody>
                    @forelse($activeSessions as $session)
                        <tr class="border-t border-gray-300 dark:border-gray-600">
                            <td class="px-4 py-2 text-gray-800 dark:text-white">{{ $session->user->name }}</td>
                            <td class="px-4 py-2 text-gray-800 dark:text-white">{{ $session->computer->label }}</td>
                            <td class="px-4 py-2 text-gray-800 dark:text-white">{{ \Carbon\Carbon::parse($session->start_time)->format('H:i d/m') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-4 py-4 text-center text-gray-500 dark:text-gray-400">Nenhuma sessão ativa no momento.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
